  ...
  <AppLayout>
    <div class="container">
      <div class="team-container">
        ...
        <a
          v-if="isRemoveMemberInputActive"
        ...
        <a
          v-if="isRemoveMemberInputActive"
          @click="isRemoveMemberInputActive = false"
          class="button"
          >Cancel</a
        >
          ...
          <div class="list" v-for="list in board.lists" :key="list.id">
            <header @click.self="handleMoveCardToList(list)">
              <input
                ...
                @keypress.enter="
                  handleEditOrDeleteList(list, $event.target.value)
                "
                :value="list.name"
                type="text"
                ...
                autofocus
              />
              <span v-else @click="list.isEditListInputActive = true">{{
                list.name
              }}</span>
            </header>
            <div class="cards">
              <div class="card" v-for="card in list.cards" :key="card.card_id">
                <div class="card-content" @click="handleClickCard(card)">
                  <input
                    ...
                    @keydown.enter="
                      handleUpdateOrDeleteCard(list, card, $event.target.value)
                    "
                    :value="card.task"
                    type="text"
                    placeholder="Are you sure want to delete?"
  ...
  methods: {
    ...
    async handleAddMember() {
      const res = await ApiService.post(
        `board/${this.$route.params.id}/member`,
        this.memberData
      );
    ...
    async handleRemoveMember() {
      const res = await ApiService.delete(
        `board/${this.$route.params.id}/member/${this.memberData.id}`
      );
    ...
    async handleStoreList() {
      const res = await ApiService.post(
        `board/${this.$route.params.id}/list`,
        this.listData
      );
    ...
    async handleStoreCard(list) {
      const res = await ApiService.post(
        `board/${this.$route.params.id}/list/${list.id}/card`,
        this.cardData
      );
    ...
    async handleEditOrDeleteList(list, value) {
      if (value !== "") {
        const res = await ApiService.put(
          `board/${this.$route.params.id}/list/${list.id}`,
          {
            name: value,
          }
        );
    ...
    async handleMoveListToRight(list) {
      const res = await ApiService.post(
        `board/${this.$route.params.id}/list/${list.id}/right`
      );
    ...
    async handleMoveListToLeft(list) {
      const res = await ApiService.post(
        `board/${this.$route.params.id}/list/${list.id}/left`
      );
    ...
    async handleUpdateOrDeleteCard(list, card, value) {
      if (value !== "") {
        const res = await ApiService.put(
          `board/${this.$route.params.id}/list/${list.id}/card/${card.card_id}`,
          {
            task: value,
          }
        );
    ...
    async handleMoveCardToList(list) {
      if (this.cardData.id !== "") {
        const res = await ApiService.post(
          `card/${this.cardData.id}/move/${list.id}`
        );
